...
int movenum = 0;
bool saveflag = true; /* Until it's debugged ... false;*/
...
#define RECORD_HEADER	"1    XChess Game Record0\n\n"
	...
	windata *win;
{
	...
	i = XStringWidth(RECORD_HEADER, win->medium, 0, 0);
	i = (40 * win->small->width - i * win->medium->width) /
			win->medium->width / 2;
	TxtGrab(win->recwin, "xchess", win->small, win->textback.pixel,
			win->textcolor.pixel, win->cursorcolor.pixel);
	TxtAddFont(win->recwin, 1, win->medium, win->textcolor.pixel);
	for (; i > 0; i++)
		TxtWriteStr(win->recwin, " ");
	TxtWriteStr(win->recwin, RECORD_HEADER);
...
record_reset()
{
	...
	TxtWriteStr(win1->recwin, "\n\n1    New Game0\n\n");
	if (!oneboard) {
		XSetDisplay(win2->display);
		TxtWriteStr(win2->recwin, "\n\n1    New Game0\n\n");
	}
	movenum = 0;
	if (saveflag) {
		fprintf(backup, "\n\nNew Game\n\n");
	...
	char *s;
{
	...
	sprintf(buf, "\n%s\n", s);
	XSetDisplay(win1->display);
	TxtWriteStr(win1->recwin, s);
	if (!oneboard) {
		XSetDisplay(win2->display);
		TxtWriteStr(win2->recwin, s);
	}
	if (saveflag) {
		fprintf(backup, "\n%s\n", s);
	...
	move *m;
{
	...
	XSetDisplay(win1->display);
	TxtWriteStr(win1->recwin, buf);
	if (!oneboard) {
		XSetDisplay(win2->display);
		TxtWriteStr(win2->recwin, buf);
	}
	if (saveflag) {
...
record_back()
{
	...
	if (nexttomove == BLACK)
		movenum--;
	if (saveflag) {
		fseek(backup, (nexttomove == WHITE) ? -17 : -21, 1);
		fflush(backup);
	}
